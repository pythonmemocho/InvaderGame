...
class Main:
    ...
    def restart(self):
        ...
        #（追加）バリアのスプライトグループを空にして、再度メソッドを実行
    ...
    def main(self):
        ...
        while running:
            ...
            #1：ゲーム実行
                ...
                #（削除）敵の弾丸とプレイヤーの衝突判定(判定条件は上と同じ)
                # if len(pg.sprite.groupcollide(self.playerSprite,self.enemybulletSprite,False,False)) > 0:
                #     self.game_condition = 3

                #(追加)敵の弾丸とプレイヤーの衝突判定＋爆発演出
                for collide in pg.sprite.groupcollide(self.playerSprite,self.enemybulletSprite,True,True):
                    explosion = Explosion(collide.rect.center)
                    self.exploSprite.add(explosion)
                    #フラグを起こす
                    self.hit = True
            ...
            #3：ゲームオーバーの時の処理
                ...
                #（削除）画面切り替えのタイマーを作業し、タイマーが切れたらゲーム状態を変更
                # self.timer -= 1
                # if self.timer < 0:

                #(追加)上記のタイマーは衝突判定の方で行っているので削除
                self.hit = False
                SCREEN.fill(BLACK)
